...
for submodule in $submodules; do
  ...
  for package in $packages; do
    ...
    if [[ -z $RACE_DETECTOR ]] || [[ $RACE_DETECTOR == "true" ]]; then
      ...
      output=$(go test $skip_options -v -short -race -count=1 -cover -coverprofile=cover.out $package $run_options 2>&1)
      TEST_RETURN_CODE=$?
    else
      # Run without the race flag
      output=$(go test $skip_options -v -short -count=1 -cover -coverprofile=cover.out $package $run_options 2>&1)
      TEST_RETURN_CODE=$?
    fi
...
# Check if coverage.txt is empty for coverage report
if [ ! -s coverage.txt ]; then
  echo "Warning: coverage.txt is empty"
fi
